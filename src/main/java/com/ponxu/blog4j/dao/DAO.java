...
public class DAO {
	...
	LOG.info("h2 数据库开始启动结束！ 物理地址为："+new File(Config.dbDir).getAbsolutePath()) ;
	...
	LOG.debug(sql);
	return DBUtils.query(mConn(), wrapper, sql, values);
    }
	...
	if (conn == null) {
	    conn = DBUtils.getConnection("jdbc:h2:" + Config.dbDir, Config.dbUser, //
		    Config.dbPassword);
